...
- **Code cites the spec for behaviour, never a design file.** A comment may still cite a design section for the *visual* surface it implements, but any comment explaining *why the logic behaves this way* points at `docs/specs/<feature>.md` — a design section anchor moves when the design is regenerated; a spec heading does not.
  - **Cite in the canonical compact form: `(docs/specs/<file>.md — <heading>)`.** It is a *pointer* to the spec heading that owns the rule, stating only the **current** behaviour. Do **not** re-list or paraphrase the spec's rules inline, and do **not** explain what the code *no longer* does or *instead of* what — the spec is the single source; the comment just points there and describes what exists. A paragraph that re-derives the rule on every surface it applies to is the smell this rule kills.
  ...
  - **A design section anchor never appears in user-facing copy.** A `§D` / `§H.3`-style design reference inside a rendered string (a hint, a paragraph, a label) is a design annotation that leaked from the mock — a user cannot resolve it. Strip it. (A section symbol followed by a **number** — `§3.5` — is legitimate document content; the smell is a section **letter** that maps to a design mock.)
